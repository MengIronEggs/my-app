...
const reLogin = () => {
  ...
  window.location.href = process.env.logout || 'https://hukou.leyanbot.com/sites/emei/logout';
};
...
request.interceptors.request.use((url: string, options: any) => {
  ...
  const urlArr = ['/login']; // 排除/login接口不做token拦截
  if (toekn || urlArr.some((item: any) => url.indexOf(item) !== -1)) {
    const host = isDev ? sessionStorage.getItem('host') : process.env.host;
